  ...
  @objc func moveLargeButton(_ sender: UIPanGestureRecognizer) {
    ...
    switch sender.state {
      case .ended:
        ...
        let nearestGridXPosition = (round((xOriginPosition) / unit)) * unit + gridView.widthOffset
        let nearestGridYPosition = (round(( yOriginPosition)/unit)) * unit + gridView.heightOffset
